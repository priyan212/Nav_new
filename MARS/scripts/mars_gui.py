#!/usr/bin/env python3
"""DINO + NavDP Mars GUI — control panel for the Habitat Marsyard sim.

Same layout and Zenoh contract as nav_pipeline/isaac_gui.py (which it is
based on), with commands tailored to the Mars environment:

- Mars object presets ("big stone", "boulder", ...) -> DINO text goals
- "Random goal": picks a random world point 4-8 m ahead (within +/-60 deg)
  ...
  ground-truth pose the habitat node publishes on mars/pose.
- "Go home": point-goal back to the spawn position.
- "Reset rover": teleports the sim rover to its start pose (mars/reset).

Run (from Nav_new root, internnav conda env, habitat_sim_node running):
    python MARS/scripts/mars_gui.py [--target "big stone"]
"""
...
PRESETS = ["big stone", "small rock", "boulder", "rock cluster", "hill", "sand mound"]
...
# ------------------------------------------------------------------------- #
class MarsPipeline(DinoNavDPPipeline):
    ...
    rover's own ego-motion (from mars/pose) and its confidence decays smoothly,
    ...
    def __init__(self, cfg: PipelineConfig = PipelineConfig(), use_depth_estimator: bool = True,
        ...
        self._belief_tick = 0
        self._belief_pose: Optional[dict] = None   # last mars/pose seen by the belief update
        self._pose_for_tick: Optional[dict] = None  # set by set_pose() before each step()

    def set_pose(self, pose: Optional[dict]) -> None:
        """Feed the current mars/pose (ground-truth rover pose) for the next
    ...
    def _step_inner(self, rgb: np.ndarray, target_text: str, depth: Optional[np.ndarray] = None,
        ...
        position is propagated by the rover's own ego-motion (mars/pose) and
        ...
        which always passes them positionally -- this Mars GUI has no REMIND
        integration, so they're unused here and always None."""
        cfg = self.cfg
        res = StepResult()
        H, W = rgb.shape[:2]
        timing = {}
...
class SharedState:
    def __init__(self, target: str):
        ...
        self.pose = None                        # {"x","z","yaw","t"} from mars/pose
        self.pose_t = 0.0
...
def pose_odom_delta(prev_pose: dict, cur_pose: dict) -> list:
    """Body-frame ego-motion [dx fwd, dy left, dtheta] between two mars/pose
...
def zenoh_setup(session: zenoh.Session, st: SharedState):
    ...
    subs = (
        ...
        + [session.declare_subscriber("mars/pose", on_pose)]
    )
    pubs = {
        ...
        "reset": session.declare_publisher("mars/reset"),
    }
    return subs, pubs
...
def inference_loop(pipe: MarsPipeline, st: SharedState, pubs, running,
    ...
    while running["on"]:
        ...
        try:
            if mode == "point":
                if world_goal is None or pose is None or pose_age > POSE_STALE_S:
                    with st.lock:
                        st.state_text = "POINT: waiting for mars/pose"
                        st.last_cmd = (0.0, 0.0)
                    time.sleep(0.2)
                    continue
        ...
        pubs["explain"].put(serialize_string(
            f"MARS DINO+NavDP [{res.state}] det={score} -> lin={res.linear:.3f} "
...
class App:
    ...
    def __init__(self, root: tk.Tk, st: SharedState, pubs):
        ...
        self.st = st
        self.pubs = pubs
        root.title("Nav_new — Mars DINO + NavDP (Habitat)")
    ...
    def random_goal(self):
        with self.st.lock:
            pose = self.st.pose
        if pose is None:
            self.status.configure(text="no mars/pose yet — is habitat_sim_node running?")
            return
        rel = math.radians(random.uniform(-RANDOM_BEARING_DEG, RANDOM_BEARING_DEG))
        dist = random.uniform(*RANDOM_DIST_RANGE)
        theta = pose["yaw"] + rel
        gx = float(np.clip(pose["x"] - dist * math.sin(theta), -WORLD_LIMIT, WORLD_LIMIT))
        gz = float(np.clip(pose["z"] - dist * math.cos(theta), -WORLD_LIMIT, WORLD_LIMIT))
        self._manual_held.clear()
        with self.st.lock:
            self.st.mode = "point"
...
def main():
    ap = argparse.ArgumentParser(description="Nav_new Mars DINO+NavDP GUI (Habitat)")
    ...
    pipe = MarsPipeline(PipelineConfig(
        device=args.device,
